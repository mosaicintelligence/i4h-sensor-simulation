...
Construction parameters: `num_angular_rays` (stored as `num_elements_x_`), `frequency` (MHz; typical IVUS values 20–40 MHz), `elevational_height` / `num_el_samples` in C++ (YAML: `elevational_height_mm` / `num_elevational_samples`; shipping PV .035 config: 1.5 mm / 8 planes — uncalibrated product default pending E3), `element_radius_mm` (typical 0.6), `focal_length_mm` (typical 4), and the standard `Pose`. The finite physical aperture in the imaging plane is represented downstream through the depth-dependent lateral PSF rather than via the ray geometry; elevational extent is sampled explicitly when `num_el_samples > 1`.
