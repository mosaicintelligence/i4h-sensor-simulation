...
2. **Alignment + un-scan-conversion** for any new diameter / gain
   combos:
   ```
   python3 i4h-sensor-simulation/tools/p035_annotate_wires.py \
       --raw-dir <NEW_DATASET>/raw
   python3 i4h-sensor-simulation/tools/p035_fit_alignment.py \
       --raw-dir <NEW_DATASET>/raw
   python3 i4h-sensor-simulation/tools/p035_unwrap.py \
       --raw-dir <NEW_DATASET>/raw
   ```

3. **Refit log_multiplier and gain LUT (E7 proper)** from Capture A's
   ~18-point gain sweep. The current
   `tools/p035_extract_gain_lut.py` does the cross-gain inference
...
4. **Refit lateral PSF (E2)** with the un-saturated low-gain frames:
   ```
   python3 i4h-sensor-simulation/tools/p035_extract_psf.py \
...
5. **Re-derive noise.sigma and ring_down.amplitude** with the new
   log_multiplier (rerun `p035_extract_gain_lut.py`; it does the
...
- Analysis tools:
  - `i4h-sensor-simulation/tools/p035_unwrap.py`
  - `i4h-sensor-simulation/tools/p035_fit_alignment.py`
  - `i4h-sensor-simulation/tools/p035_extract_psf.py`
  - `i4h-sensor-simulation/tools/p035_extract_tgc.py`
  - `i4h-sensor-simulation/tools/p035_extract_noise.py`
  - `i4h-sensor-simulation/tools/p035_extract_ringdown.py`
  - `i4h-sensor-simulation/tools/p035_extract_gain_lut.py`
  - `i4h-sensor-simulation/tools/p035_polar_utils.py` (shared helpers)
