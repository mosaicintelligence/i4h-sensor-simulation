...
# paired-polar bench reference migrated from the legacy P_035
...
BENCH_POLAR_DIR = (WORKSPACE_ROOT / "ivus_test_0515" / "raw" / "b2_w_wire_p1" / "derived" / "polar")
BENCH_FRAMES_META = (WORKSPACE_ROOT / "ivus_test_0515" / "raw" / "b2_w_wire_p1" / "derived"
    / "frames_meta.csv")
...
WAVE0_SPECKLE_PATHS = tuple(WAVE0_ROOT / "raw" / sub / "derived" / "speckle" / "speckle_summary.json"
...
WAVE0_SPECKLE_PATHS_E5_LEGACY = tuple(WAVE0_ROOT / "raw" / sub / "derived" / "speckle" / "speckle_summary.json"
...
class TestResult:
    ...
    summary: str
    detail: dict[str, Any]
    # Phase 4: tag each test as parameter (gating) or
...
def _extract_phenomenology_companions(parent: TestResult) -> list[TestResult]:
    """— emit derived `bank=phenomenology` companions for
    ...
    * ``M. Speckle`` --> legacy palette CoV_log + per-frame palette
    ...
    if parent.name.startswith("F. Noise floor"):
        ...
        if isinstance(blk, dict) and blk.get("per_pair"):
            ...
            summary = (f"E6 anechoic-water envelope-domain diagnostic across "
            ...
            if isinstance(agg, dict):
                ...
                if bench_med is not None and sim_med is not None:
                    summary += (f" Aggregate: sim ff_std = {sim_med:.2f} palette "
                        f"vs bench {bench_med:.2f} palette "
                        f"(rel.err median = {(rel or 0)*100:.0f}%).")
            out.append(TestResult(name="F-diag. E6 anechoic-water envelope diagnostic "
                     "(informational)",
                status="diagnostic",
                summary=summary,
                detail=blk,
        ...
        if isinstance(b2c, dict):
            ...
            summary = (f"B2c E2E milk anchor at slider 68 D = 15 mm: sim "
            ...
            out.append(TestResult(name="B2c-diag. Gain E2E milk anchor "
                     "(informational)",
                status="diagnostic",
                summary=summary,
                detail=b2c,
        ...
        if isinstance(leg, dict):
            ...
            summary = (f"Legacy palette-domain speckle diagnostic vs E4a Wave 0 "
            ...
            out.append(TestResult(name="M-diag. Legacy palette CoV_log + radial_corr "
                     "(informational)",
                status="diagnostic",
                summary=summary,
                detail=leg,
...
def make_probe_at(cfg, position_mm=(0.0, 0.0, 0.0), rotation_rad=(0.0, 0.0, 0.0)):
    ...
    pose = rs.Pose(np.array(position_mm, dtype=np.float32),
        np.array(rotation_rad, dtype=np.float32),)
    return rs.IVUSProbe(pose,
...
B2_WIRE_POSITIONS = ((1, 4.000, 0.000), (2, 5.196, 3.000), (3, 4.000, 6.928),
...
def render_frames(cfg, world, materials, n_frames: int, sim_params=None,
    ...
    (this is a bug that previously broke Test F / Test M sim-side
    radial autocorrelation and r-band masking).

    ``coherent_scatter`` flag:
...
def b_mode_to_theta_r(frame: np.ndarray, cfg) -> np.ndarray:
    ...
    if frame.shape == (n_r, n_theta):
        return frame.T
    raise RuntimeError(f"Unexpected b_mode shape {frame.shape}; expected (theta, r)="
    ...
    for row in data.get("frames", []):
        if row.get("file") == bare or row.get("file") == name:
            return {
                ...
                "apparatus_offset_xy_mm_math": (float(row["apparatus_offset_from_image_center_mm"][0]),
...
def transform_design_to_bench_polar(x_mm: float, y_mm: float, alignment: dict,) -> tuple[float, float]:
...
def load_bench_polar(name: str = "FILE0005.dcm") -> tuple[np.ndarray, dict]:
    ...
    * Wave 0 B2 polar dir uses ``FILE0000.dcm.npy`` (preserves the DICOM
    ...
    """
    candidate_paths = (BENCH_POLAR_DIR / f"{name}.npy",
    ...
    for path in candidate_paths:
        ...
        raise FileNotFoundError(f"No bench polar.npy found for '{name}' in {BENCH_POLAR_DIR} "
            ...
            for row in csv.DictReader(f):
                if row["file"] in lookup_keys:
                    ...
                    meta.update({
                        ...
                        "ar_state": bool(int(_maybe_float(row.get("ar_state", row.get("mode_flag", "0")), 0.0))),
                    })
                    break
    return arr, meta
    ...
    *apparatus* frame using the same polar convention as
    ...
    for (idx, x_mm, y_mm) in B2_WIRE_POSITIONS:
        if alignment is None:
            ...
            theta_polar_deg, r = transform_design_to_bench_polar(x_mm, y_mm, alignment)
...
def save_paired_polar_figure(sim_calibrated_stack: np.ndarray, bench: np.ndarray,
    ...
    * **Bench:** same palette [11, 239] with γ = 0.6 on intermediate values
    ...
    fig.suptitle("Wire-phantom polar B-mode (probe at centre, r tick = mm)",
        fontsize=11,)
    plt.tight_layout()
...
def save_paired_inner_figure(sim_anechoic: np.ndarray, bench: np.ndarray,
...
def test_config_round_trip(cfg, sim_params) -> TestResult:
    ...
    tgc_ok = len(yaml_pts) == len(sim_pts) and all(abs(float(y[0]) - float(s[0])) <= max(1e-6, 1e-5 * max(abs(float(y[0])), abs(float(s[0])))) and
    ...
    if issues:
        return TestResult("A. Configuration-sheet round-trip",
            "fail",
...
def test_self_consistency(cfg) -> TestResult:
    ...
    if issues:
        return TestResult("B. Configuration self-consistency",
            "fail",
...
def resolve_psf_anchor(anchor: str) -> dict:
    ...
    if anchor == "wave0":
        if not WAVE0_PSF_FIT_PATH.is_file():
            raise FileNotFoundError(f"Wave 0 anchor missing at {WAVE0_PSF_FIT_PATH}. Run "
...
def test_psf(cfg, sim_params, materials, n_frames: int, out_dir: Path,
    ...
    * **diagnostic** (4 rotations x n_frames frames, ``log_floor = 1e-19``,
    ...
    gain_db_diag = {
        "available": False,
        "rationale": ("INFORMATIONAL diagnostic: per-wire gain_db_required_to_match_"
    ...
    for s in summary:
        ...
        bench_med, bench_p25, bench_p75, n_b = _bench_stats_for_r(bench_axial_clean, s["r_mm"], half_window_mm=1.0, key="axial_fwhm_mm")
    ...
    axial_summary = (f"B2 spiral phantom, {len(B2_WIRE_INDICES_USED)} scored wires (w2..w11), "
        ...
        ", ".join((f"r={s['r_mm']:.1f}: {(s['axial_fwhm_mm_median']*1000):.0f} um"
    ...
    for s in summary:
        ...
        if s.get("lateral_fwhm_arc_mm_median") is None:
            continue
        bench_med, bench_p25, bench_p75, n_b = _bench_stats_for_r(bench_lat_clean, s["r_mm"], half_window_mm=1.0,
    ...
    if valid_lat:
        ...
        lateral_detail["focal_trend_informational"] = ("constant_angular kernel has no focal minimum within the "
    ...
    lateral_summary = (f"B2 spiral phantom, {len(B2_WIRE_INDICES_USED)} scored wires (w2..w11). "
        ...
        ", ".join((f"r={s['r_mm']:.1f}: {s['lateral_fwhm_arc_mm_median']:.2f} mm"
    ...
    if plt is not None:
        try:
            ...
            slider_offset_db = ((bench_slider - PAIRED_POLAR_REFERENCE_SLIDER)
            ...
            paired_frames = render_frames(cfg, world_paired, materials, n_frames,
                                            sim_paired)
            sim_paired_stack = np.stack([b_mode_to_theta_r(f, cfg) for f in paired_frames])
            bench_alignment = load_bench_alignment(BENCH_REFERENCE_FRAMES[0])
            bench_wires = load_bench_wire_positions(max_radius_mm=25.0, alignment=bench_alignment)
            save_paired_polar_figure(sim_paired_stack, bench_arr,
    ...
    return (TestResult("C. Axial PSF (per radius)", axial_status, axial_summary, axial_detail),
        TestResult("D. Lateral PSF (per radius)", lat_status, lateral_summary, lateral_detail),)


E6_RINGDOWN_DIR = (WORKSPACE_ROOT / "ivus_test_0508" / "raw" / "c_take2_water"
    / "derived" / "ringdown")
...
def _load_e6_ringdown_pairs() -> tuple[list[dict], dict]:
    ...
    if not summary_path.is_file():
        raise FileNotFoundError(f"E6 ringdown summary missing: {summary_path}. "
    ...
    if not pairs:
        raise RuntimeError(f"No usable E6 ringdown pairs found under {E6_RINGDOWN_DIR}.")
    ...
    *Pass criterion.* Median across all pairs:
    ...
    try:
        for pair in pairs:
            ...
            rms_inner = float(np.sqrt(np.mean((mean_aline_sim[inner_mask] - bench_on_sim[inner_mask]) ** 2)))
    ...
    if plt is not None:
        ...
        fig.suptitle(f"Ring-down — sim vs E6 AR-off, pooled across {len(pair_results)} "
    ...
    detail = {
        ...
        "bench_source": ("ivus_test_0508/raw/c_take2_water/derived/ringdown/"
            "ringdown_summary_v2.json + AR-off.npy A-lines (E6 anechoic)"),
    }
    pair_labels = ", ".join(f"g{p['gain']:.0f}D{p['diameter_mm']:.0f}" for p in pair_results)
    summary = (f"E6 ringdown anchor: {len(pair_results)} (gain, D) pairs "
        f"({pair_labels}). "
...
# pull the multi-frame DICOM corresponding to a given (gain, D,
...
def _load_capture_polar(manifest_path: Path,
                        ...
                        num_theta: int = 256,
                        label: str = "capture",) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    if not manifest_path.is_file():
        raise FileNotFoundError(f"{label} manifest missing: {manifest_path}. Stage the "
    ...
    if match is None:
        ...
        raise RuntimeError(f"No {label} manifest row for gain={gain}, D={diameter_mm}"
...
def _load_e6_capture_polar(gain: float, diameter_mm: float, ar_state: str,
                           r_lo_mm: float = 0.5, r_hi_mm: float = 22.0,
                           num_theta: int = 256) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    return _load_capture_polar(manifest_path=capture_dir / "manifest_subset.csv",
...
# E4a multi-frame milk loader. Unlike E6 (single capture folder
...
def _load_e4a_capture_polar(gain: float, diameter_mm: float,
                            take: int | str = "auto",
                            r_lo_mm: float = 0.5, r_hi_mm: float = 22.0,
                            num_theta: int = 256,) -> tuple[np.ndarray, np.ndarray, float, str]:
    ...
    for folder_name in candidates:
        ...
        try:
            polar, r_mm, px_mm = _load_capture_polar(manifest_path=capture_dir / "manifest_subset.csv",
                dcm_root=capture_dir,
            ...
            last_err = exc
    raise RuntimeError(f"No E4a take contains gain={gain}, D={diameter_mm} "
        f"(searched {candidates}; last error: {last_err})")


# E4c multi-frame loader (1:1 diluted milk). Same layout as
# E4a: three takes p1/p2/p3, slider grid varies per take, ar_state always
# "off". This is the phantom that the bench LUT was anchored
...
def _load_e4c_capture_polar(gain: float, diameter_mm: float,
    ...
    for folder_name in candidates:
        ...
        try:
            ...
            last_err = exc
    raise RuntimeError(f"No E4c take contains gain={gain}, D={diameter_mm} "
        f"(searched {candidates}; last error: {last_err})")
...
def _multiframe_noise_stats_from_polar(polar: np.ndarray,
                                       ...
                                       reject_softness: float = 0.0,) -> dict[str, float]:
    ...
    the envelope-domain residual is the invertible / confounder-
    ...
    if has_log_chain:
        env_stats = _envelope_residual_std(band, log_multiplier=float(log_multiplier), # type: ignore[arg-type]
...
def _e6_multiframe_noise_stats(gain: float, diameter_mm: float,
    ...
    return _multiframe_noise_stats_from_polar(polar, r_mm, r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm,
...
def _e4a_multiframe_noise_stats(gain: float, diameter_mm: float = 30.0,
    ...
    The bench LUT calibration was anchored on **e4c**
    ...
    """
    polar, r_mm, _px_mm, take_id = _load_e4a_capture_polar(gain, diameter_mm, take=take)
    out = _multiframe_noise_stats_from_polar(polar, r_mm, r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm,
        log_multiplier=log_multiplier, log_floor=log_floor,
        reject_palette=reject_palette,
        saturation_palette=saturation_palette,
        reject_softness=reject_softness,)
...
def _e4c_multiframe_noise_stats(gain: float, diameter_mm: float = 30.0,
    ...
    This is the phantom against which the LUT and ``noise.sigma``
    ...
    The returned dict has an extra ``take_id`` field ("p1"/"p2"/"p3").
    """
    polar, r_mm, _px_mm, take_id = _load_e4c_capture_polar(gain, diameter_mm, take=take)
    out = _multiframe_noise_stats_from_polar(polar, r_mm, r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm,
        log_multiplier=log_multiplier, log_floor=log_floor,
        reject_palette=reject_palette,
        saturation_palette=saturation_palette,
...
# ----------------------------------------------------------------------------
# envelope-domain noise residual.
# ----------------------------------------------------------------------------
...
# signal lifting the local mean). This is the mechanism behind the # over-calibration that had to undo: matching ff_std_palette against
...
def _palette_to_envelope(palette: np.ndarray,
                         ...
                         reject_softness: float = 0.0,) -> np.ndarray:
...
def _envelope_residual_std(polar_palette_stack: np.ndarray,
                           ...
                           saturation_margin_palette: float = 10.0,) -> dict[str, float]:
    ...
    if stack.ndim < 2:
        raise ValueError(f"_envelope_residual_std expects (n_frames,...) stack; got "
            f"shape={stack.shape}.")
    palette_mean = stack.mean(axis=0)
    soft_floor = float(reject_palette) + (float(reject_softness) * np.log(2.0) if reject_softness > 0.0 else 0.0)
    valid = (palette_mean > soft_floor + floor_margin_palette) & (palette_mean < float(saturation_palette) - saturation_margin_palette)
    ...
    env_stack = _palette_to_envelope(stack, log_multiplier=log_multiplier, log_floor=log_floor,
...
# ----------------------------------------------------------------------------
# Bench slider->dB curve (linear, ~1 dB/slider)
# ----------------------------------------------------------------------------
# An earlier model treated the bench curve as piecewise-sublinear, with
# rate falling from 1.0 dB/slider near slider 45 to 0.51 dB/slider near
# slider 65. The 0.51 dB/slider anchor was derived from E4a milk-band
# palette changes at slider 62 -> 68, but the milk palette in that range
# is partially LUT-compressed (sitting in the soft-reject knee at palette
# 20..40 where the display LUT has a non-linear floor lift) -- so what
# looked like sublinear analog gain was actually LUT compression.
#
# `_pass25c_bench_full_lut.py` audits the full bench dataset and shows
# the bench's slider->dB curve is essentially LINEAR
...
# slider 57 -> +3.0 dB
# slider 62 -> +8.0 dB
# slider 68 -> +14.0 dB <- Test I/M anchor
# slider 70 -> +16.0 dB
#
# An earlier sublinear bench model was paired with an over-strong noise
# floor so the two errors partially cancelled at slider 68 milk; the
# current linear slider->dB curve and the calibrated noise floor fix
# both jointly (see volcano_s5i.yaml envelope_noise comment block for
# the noise side).
#
...
#
# The low-slider anchors (s < 22) are the primary correction: an
# earlier flat-rate model extrapolated 1.0 dB/slider down to slider 0,
# but the bench shows ~0.4 dB/slider at sliders 2-6, rising to 1.0 only
# above slider 14.
...
def slider_to_db(slider: float, ref_slider: float = 54.0) -> float:
    ...
    curve: linear ~1.0 dB/slider above slider ~14 (derived from
    ...
    happen to agree across the calibrated flat-rate anchors, but the helper
...
def _incoherent_2d_corr_lengths_mm(polar_frames: np.ndarray,
    ...
    Concern 1 -- the legacy ``_radial_corr_length_mm`` is a 1-D
    ...
    if polar.ndim != 3:
        raise ValueError(f"_incoherent_2d_corr_lengths_mm expects (frames, theta, r), "
...
def _render_noise_distribution_paired_figure(*, sim_full_stack: np.ndarray,
    ...
    envelope_noise_sigma: float,) -> None:
    ...
    Layout (2 rows x 3 columns):
      ...
      * (1,1) Per-pixel frame-to-frame std histogram (INCOHERENT noise), bench
                vs sim, overlaid -- the key distribution the envelope
    ...
    # 60-78% coherent variance characterised in the noise
    ...
    bench_incoherent_frame = (bench_polar[bench_rep] - bench_temporal_mean + bench_global_mean)
    ...
    render_cart_bmode(ax_b_img, bench_incoherent_frame, t_far_mm=t_far_mm,
        title=(f"Bench INCOHERENT noise example\n"
    ...
    render_cart_bmode(ax_s_img, sim_full_stack[sim_rep], t_far_mm=t_far_mm,
        title=(f"Sim example (gain_db {sim_gain_db:+.1f})\n"
    ...
    combo = np.concatenate([bench_ff_std_map, np.full_like(bench_ff_std_map[:, :3], np.nan),
    ...
    fig.suptitle(f"Test F -- Noise floor distribution paired figure "
...
def _render_milk_anchor_paired_figure(*, milk_block: dict,
    cfg, sim_params, materials,
    n_frames: int,
    out_path: Path,
    rep_slider: float = 68.0,
    rep_diameter_mm: float = 15.0,
    ref_slider: float = 54.0,) -> None:
    """Paired figure for the milk envelope-domain noise anchor (Test F
    primary gate).
    ...
    tol = float(milk_block.get("params", {}).get("rel_err_tol", MILK_ANCHOR_REL_ERR_TOL))

    # Find the representative anchor that matches rep_slider; fall back
    # to the slider with the highest E4c valid fraction.
    rep_idx = next((i for i, a in enumerate(per_anchor) if abs(a["slider"] - rep_slider) < 0.5),
        None,)
    if rep_idx is None:
    ...
    try:
        bench_e4c_polar, bench_e4c_r_mm, _, _ = _load_e4c_capture_polar(rep_slider_actual, rep_diameter_mm, take="auto")
    ...
    try:
        ...
        # match test_noise's coherent_scatter mode so the figure
    ...
    min_vf = float(milk_block.get("params", {}).get("min_valid_fraction", MILK_ANCHOR_MIN_VALID_FRACTION))
    ...
    for s, b, v in zip(sliders, bench_e4c_y, e4c_valid):
        if v >= min_vf and np.isfinite(b):
            ax_curve.fill_between([s - 1.5, s + 1.5], [b * (1 - tol)] * 2, [b * (1 + tol)] * 2,
    ...
    for s, sim_v, b, r, v in zip(sliders, sim_y, bench_e4c_y, e4c_rel, e4c_valid):
        if v >= min_vf and np.isfinite(r):
            ax_curve.annotate(f"E4c {r*100:.0f}%",
    ...
    ax_curve.set_title(f"Test F primary -- milk anchor sweep over device slider "
    ...
    if bench_e4c_polar is not None and bench_e4c_r_mm is not None:
        ...
        bench_resid_std = float((rep_anchor["bench_e4c"] or {}).get("envelope_residual_std",
                                                 float("nan")))
        bench_palette_mean = float((rep_anchor["bench_e4c"] or {}).get("mean_palette", float("nan")))
        ax_b_img.imshow(bench_band, aspect="auto", cmap="gray",
                        vmin=vmin_disp, vmax=vmax_disp,
                        extent=(r_lo, r_hi, bench_band.shape[0], 0))
        ax_b_img.set_title(f"Bench E4c slider {rep_slider_actual:.0f} D={rep_diameter_mm:.0f}\n"
    ...
    if sim_polar is not None and sim_r_mm is not None:
        ...
        sim_resid_std = float((rep_anchor["sim"] or {}).get("envelope_residual_std",
                                            float("nan")))
        sim_palette_mean = float((rep_anchor["sim"] or {}).get("mean_palette", float("nan")))
        ...
        ax_s_img.set_title(f"Sim slider {rep_slider_actual:.0f} (gain_db "
    ...
    def _envelope_per_pixel_std(polar: np.ndarray, r_mm: np.ndarray) -> np.ndarray:
        ...
        env = _palette_to_envelope(band,
            log_multiplier=float(cfg.processing.log_multiplier),
            log_floor=float(cfg.processing.log_floor),
            reject_palette=float(cfg.processing.reject_palette),
            reject_softness=float(getattr(cfg.processing, "reject_palette_softness", 0.0)),)
    ...
    if bench_std_map is not None and sim_std_map is not None:
        ...
        im = ax_stdmap.imshow(np.ma.masked_invalid(combo),
        ...
        ax_stdmap.set_title(f"Per-pixel temporal std (envelope domain)\n"
    ...
    fig.suptitle(f"Test F primary -- milk envelope-residual sweep "
...
# ----------------------------------------------------------------------------
# Test F primary gate: multi-slider milk envelope-domain anchor.
# ----------------------------------------------------------------------------
...
# the same noise.sigma to different palette stds). See the discussion
# in this file and in ``tier1_self_validate.py``.
#
...
# * E4c (1:1 milk-water): same phantom used for the LUT
...
def _test_noise_milk_anchors(cfg, sim_params, materials, n_frames: int,
    ...
    try:
        for slider in MILK_ANCHOR_SLIDERS:
            ...
            try:
                bench_e4c = _e4c_multiframe_noise_stats(slider, MILK_ANCHOR_DIAMETER_MM,
            ...
            try:
                bench_e4a = _e4a_multiframe_noise_stats(slider, MILK_ANCHOR_DIAMETER_MM,
                    r_lo_mm=r_lo, r_hi_mm=r_hi,
                    log_multiplier=log_mult, log_floor=log_floor,
                    reject_palette=reject, saturation_palette=sat,
                    reject_softness=softness)
            ...
            # use coherent_scatter=True so the sim's per-frame
            ...
            sim_stats = _multiframe_noise_stats_from_polar(bg_frames, r_mm_sim,
                r_lo_mm=r_lo, r_hi_mm=r_hi,
                log_multiplier=log_mult, log_floor=log_floor,
                reject_palette=reject, saturation_palette=sat,
                reject_softness=softness)
    ...
    # --- Aggregate. Primary uses E4c; E4a is a
...
def test_noise(cfg, sim_params, materials, n_frames: int,
    ...
    *paradigm shift.* Test F is now anchored on **E4c
    ...
    stds in sim vs bench -- the operating-point bias that the over-calibration revealed and that motivated this rewrite).
    ...
    *Legacy E6 anechoic-water diagnostic.* Bench source:
    ...
    domain diagnostics, but NONE of these gate Test F --
    ...
    # =================================================================
    # PRIMARY -- multi-slider milk anchor (gates PASS/FAIL).
    # =================================================================
    milk_block = _test_noise_milk_anchors(cfg, sim_params, materials, n_frames=n_frames, ref_slider=ref_slider)
    ...
    for pair in pairs:
        ...
        # multi-frame stats (proper per-pixel frame-to-frame
        ...
        # pass log-chain params so we also get the envelope-
        ...
        try:
            on_mf = _e6_multiframe_noise_stats(pair["gain"], pair["diameter_mm"], "on",
            ...
            off_mf = _e6_multiframe_noise_stats(pair["gain"], pair["diameter_mm"], "off",
                log_multiplier=_log_mult, log_floor=_log_floor,
                reject_palette=_reject, saturation_palette=_saturation,
                reject_softness=_softness,)
        except (FileNotFoundError, RuntimeError) as exc:
        ...
        try:
            if (abs(pair["gain"] - 50.0) < 0.5
                ...
                bench_polar_canon, bench_r_canon, _ = _load_e6_capture_polar(pair["gain"], pair["diameter_mm"], "on")
                bench_2d_corr = _incoherent_2d_corr_lengths_mm(bench_polar_canon, bench_r_canon)
    ...
    if not usable_pairs:
        raise RuntimeError("No usable E6 AR-on pairs above the reject floor. Stage more "
    ...
    try:
        for bp in usable_pairs:
            ...
            band = []
            # match the bench's deep-tail r-band exactly so
            ...
            sim_r_hi_mm = 9.84
            full_frames_for_fig: list[np.ndarray] | None = ([] if (abs(bp["gain"] - canonical_g) < 0.5
            ...
            sim_r_band_mm = (np.arange(sim_aline_mean.shape[0], dtype=np.float64)
                             + r_lo) * dr
            sim_corr = _radial_corr_length_mm(sim_aline_mean, sim_r_band_mm,
                r_lo_mm=sim_r_lo_mm, r_hi_mm=sim_r_hi_mm)
            # 2-D incoherent autocorrelation on the sim band.
            ...
            try:
                ...
                sim_corr_2d = _incoherent_2d_corr_lengths_mm(band, sim_r_band_mm,
            ...
            # compute per-pixel frame-to-frame std AS WELL as
            ...
            # envelope-domain residual on the sim side, for
            ...
            # see narrative).
            sim_env_stats = _envelope_residual_std(band,
                log_multiplier=float(cfg.processing.log_multiplier),
                log_floor=float(cfg.processing.log_floor),
                reject_palette=float(cfg.processing.reject_palette),
            ...
            # prefer the apples-to-apples per-pixel frame-to-
            ...
            # 2-D incoherent autocorrelation comparison.
    ...
    # 2-D incoherent autocorrelation aggregates (only computed
    ...
    # pooled envelope-domain residual diagnostic. Reported
    ...
    for p in per_pair_results:
        ...
        # at slider 50 under the noise budget -- sim palette ~12
    ...
    # legacy E6 metrics are INFORMATIONAL only. The primary
    ...
    if milk_n_e4c < 2 and milk_n_e4a < 2:
        ...
        # bench data wasn't sufficient at the current operating
    ...
    detail = {
        # ---- PRIMARY: milk envelope-domain anchor -----------
        "milk_anchor_primary": milk_block,
        # ---- LEGACY: E6 anechoic-water diagnostic ----------
        "legacy_e6_diagnostic": {
            ...
            "bench_source": ("ivus_test_0508/raw/c_take2_water/derived/ringdown/ringdown_summary_v2.json "
        ...
        "noise_yaml": {
            ...
            "envelope_noise_reference_gain_db": float(cfg.processing.envelope_noise.reference_gain_db),
            "noise_sigma_legacy": float(cfg.processing.noise.sigma),
        },
        "bench_source_primary": ("ivus_test_0515/raw/e4c_milk_water_gain_p{1,2,3}/manifest_subset.csv "
    ...
    pair_labels = ", ".join(f"g{p['gain']:.0f}D{p['diameter_mm']:.0f}" for p in per_pair_results)
    ...
    for a in milk_per:
        ...
        if np.isfinite(b_v) and b_v > 0:
            e4c_anchor_lines.append(f"g{a['slider']:d}(bench={b_v:.3f} sim={s_v:.3f} "
                f"rel={a['rel_err_e4c']*100:.0f}% vf={b_vf*100:.0f}%)")
        else:
            e4c_anchor_lines.append(f"g{a['slider']:d}(skipped: bench n/a or vf<5%)")
    milk_anchor_summary = (f"PRIMARY (milk anchor, gates): "
    ...
    legacy_summary = (f"LEGACY E6 DIAGNOSTIC (informational, not gating): "
    ...
    yaml_summary = (f"YAML: envelope_noise.{{mean,sigma}} = "
    ...
    # paired noise-distribution figure (sim vs bench example
    ...
    if (out_dir is not None and plt is not None and fig_capture is not None):
        try:
            ...
            bench_polar, bench_r_mm, _ = _load_e6_capture_polar(fig_capture["gain"], fig_capture["diameter_mm"], "on")
            bench_band_mask = ((bench_r_mm >= 4.0) & (bench_r_mm <= 9.84))
            bench_mf = _e6_multiframe_noise_stats(fig_capture["gain"], fig_capture["diameter_mm"], "on",
                log_multiplier=float(cfg.processing.log_multiplier),
                log_floor=float(cfg.processing.log_floor),
                reject_palette=float(cfg.processing.reject_palette),
                saturation_palette=float(cfg.processing.saturation_palette),
            ...
            _render_noise_distribution_paired_figure(sim_full_stack=fig_capture["sim_full_stack"],
    ...
    # follow-up: paired milk-anchor figure (primary gate
    ...
    if out_dir is not None and plt is not None:
        try:
            ...
            _render_milk_anchor_paired_figure(milk_block=milk_block,
            ...
            detail["milk_anchor_figure"] = str(milk_fig_path.relative_to(out_dir))
        except Exception as exc: # pragma: no cover -- figure is decorative
...
def test_log_compression(cfg, sim_params, materials, out_dir: Path) -> TestResult:
    ...
    The / K2v2 kernel implements the spec mapping directly::
    ...
    if max_abs <= 3.0:
        ...
        status = "fail"
        summary = (f"Synthetic envelope sweep: |Δpalette| up to {max_abs:.1f} > 3 between the "
...
# ----------------------------------------------------------------------------
# Test gain alignment: 3-sub-test decomposition.
# ----------------------------------------------------------------------------
...
#
# decomposes gain alignment into three orthogonal sub-tests, each
...
def _test_gain_b2a_function_check(cfg) -> dict:
    ...
    return {
        ...
        "bench_source": ("tier1_results/pass25c_bench_full.json :: "
...
def _test_gain_b2b_sim_delta(cfg, sim_params, materials, n_frames: int,
    ...
    # saturation ceiling (239) and free of the soft-reject knee. # B2b operates in palette_pre (post-inversion) so the shift is exactly
    ...
    env_lo = _palette_to_envelope(band_lo, log_multiplier=log_mult, log_floor=log_floor,
        reject_palette=reject, reject_softness=softness)
    env_hi = _palette_to_envelope(band_hi, log_multiplier=log_mult, log_floor=log_floor,
        reject_palette=reject, reject_softness=softness)
...
def _test_gain_b2c_milk_e2e(cfg, sim_params, materials, n_frames: int,
    ...
    # `mean_palette`.
    try:
        bench = _e4c_multiframe_noise_stats(slider, diameter_mm,
            r_lo_mm=r_lo, r_hi_mm=r_hi,
            log_multiplier=log_mult, log_floor=log_floor,
            reject_palette=reject, saturation_palette=sat,
            reject_softness=softness)
    ...
    dr = float(cfg.sim.t_far_mm) / n_r
    r_mm_sim = (np.arange(n_r, dtype=np.float64) + 0.5) * dr
    sim = _multiframe_noise_stats_from_polar(bg_frames, r_mm_sim,
        r_lo_mm=r_lo, r_hi_mm=r_hi,
        log_multiplier=log_mult, log_floor=log_floor,
        reject_palette=reject, saturation_palette=sat,
        reject_softness=softness)
    ...
    return {
        ...
        "bench_envelope_residual_std": float(bench.get("envelope_residual_std", float("nan"))),
        "sim_envelope_residual_std": float(sim.get("envelope_residual_std", float("nan"))),
        "world": "build_uniform_milk_world",
        "bench_source": ("ivus_test_0515/raw/e4c_milk_water_gain_p{1,2,3} :: "
...
def test_gain_alignment(cfg, sim_params, materials, out_dir: Path,
    ...
    """B2 — Gain alignment, decomposed into three sub-tests.
    ...
    b2b = _test_gain_b2b_sim_delta(cfg, sim_params, materials, n_frames=n_frames, ref_slider=ref_slider)
    ...
    b2c = _test_gain_b2c_milk_e2e(cfg, sim_params, materials, n_frames=n_frames, ref_slider=ref_slider)
    ...
    if any(s == "n/a" for s in sub_statuses):
        ...
        failed = [name for name, st in zip(("B2a", "B2b", "B2c"), sub_statuses)
                  if st == "fail"]
        status = "partial"
        status_basis = (f"B2b passes (sim pipeline structurally correct) but "
    ...
    summary = (f"B2a function-check vs bench wire peaks: {b2a['status']} "
        f"(median |dB err| = "
        f"{b2a.get('median_abs_err_dB', float('nan')):.2f} dB, "
    ...
    detail = {
        ...
        "yaml_knobs": {
            ...
            "reject_palette": float(cfg.processing.reject_palette),
            "saturation_palette": float(cfg.processing.saturation_palette),
            "reject_palette_softness": float(getattr(cfg.processing, "reject_palette_softness", 0.0)),
            "gain_db": float(cfg.processing.gain_db),
        },
    }
    return TestResult("B2. Gain alignment (B2a function / B2b sim-internal / B2c milk E2E)",
        status, summary, detail,)


# ---------------------------------------------------------------------------
# Legacy gain-alignment block -- retained for reference but no longer called.
# The decomposition above (B2a/B2b/B2c) replaces this. Kept here in
# case a future debugging session wants to cross-check the bench E6 AR-off
# water-tail measurement; remove this whole function once has bedded
# in.
def _legacy_test_gain_alignment(cfg, sim_params, materials, out_dir: Path,
    ...
    usable_pairs = [b for b in bench_per_pair if not b["at_floor"]]
    if not usable_pairs:
        raise RuntimeError("No usable E6 AR-off pairs above the reject floor. Stage more "
    ...
    detail = {
        ...
        "interpretation": ("Pooled |sim_bg - bench_AR_OFF| across the usable E6 pairs. AR is "
    ...
    pair_labels = ", ".join(f"g{p['gain']:.0f}D{p['diameter_mm']:.0f}" for p in per_pair_results)
    summary = (f"E6 AR-off bg anchor: {len(per_pair_results)} usable (gain, D) pairs "
        f"({pair_labels}). "
    ...
    return TestResult("Gain alignment (E6 anechoic AR-off, multi-pair)",
        status, summary, detail,)
...
def _bench_milk_depth_profile(*, ref_gain: float = 68.0,
    ...
    try:
        ...
        raise RuntimeError(f"Cannot import unwrap helpers from {HERE} (needed for E4a milk "
            f"polar unwrap): {exc}") from exc
    ...
    if not pooled:
        raise RuntimeError(f"No E4a milk frames matched slider={ref_gain}, D={ref_diameter_mm} "
...
def _bench_depth_profile(*, exclude_angle_deg: float = 12.0,
    ...
    if not profiles or pix_pitch_mm is None:
        raise RuntimeError(f"No bench frames at gain={gain_slider}, D={diameter_mm} found in "
            f"{BENCH_POLAR_DIR}")
    ...
    *Protocol-correct bench medium: E4a uniform evaporated-milk phantom.*
    ...
    Implementation:
      * Renders ``n_frames`` ``build_uniform_milk_world`` frames at the
        ...
        milk speckle above reject_palette). calibration: the
    ...
    """
    world = build_uniform_milk_world(materials)
    # use the MEASURED slider->dB curve (sublinear above slider 50).
    ...
    bench_r_mm, bench_mean_per_r, bench_std_per_r, bench_n_frames, mask_meta = (_bench_milk_depth_profile(ref_gain=bench_slider, ref_diameter_mm=60.0))
    ...
    if plt is not None:
        ...
        try:
            ...
            if _match is not None and (e4a_dir / _match["file"]).is_file():
                ...
                bench_example_polar = polar_resample(_arr, _cx_px, _cy_px, _pix, 0.0, 1, 360, _num_r, _dr_mm)
        ...
        ax_shape.set_title(f"Shape comparison (de-meaned in eval band)\n"
        ...
        fig.suptitle(f"Test I -- Depth uniformity in uniform milk bath "
    ...
    summary = (f"Milk sim vs E4a bench mean palette over r ∈ "
...
# ---------------------------------------------------------------------------
# Wave 0 E5 speckle / cyst anchor — Test M
# ---------------------------------------------------------------------------
...
def _load_wave0_speckle_anchor() -> dict | None:
    ...
    return {
        ...
        "lateral_corr_flagged": ("clipped at autocorrelation max-lag (uniform milk has very long "
...
# ----------------------------------------------------------------------------
# Test M residual-domain speckle metric (Phase 3 reformulation).
# ----------------------------------------------------------------------------
...
def _speckle_residual_metrics(polar_stack: np.ndarray,
    ...
    if polar_stack.ndim != 3:
        raise ValueError(f"polar_stack must be (n_frames, n_theta, n_r); got {polar_stack.shape}")
...
def _bench_speckle_residual_e4(slider: float, diameter_mm: float,
    ...
    """
    if phantom == "e4c":
        polar, r_mm, _px_mm, take_id = _load_e4c_capture_polar(slider, diameter_mm, take=take)
    elif phantom == "e4a":
        polar, r_mm, _px_mm, take_id = _load_e4a_capture_polar(slider, diameter_mm, take=take)
    else:
        raise ValueError(f"Unknown phantom {phantom!r} (expected 'e4a' or 'e4c')")
    n_theta = polar.shape[1]
    dtheta_rad = 2.0 * math.pi / n_theta
    out = _speckle_residual_metrics(polar, r_mm, dtheta_rad=dtheta_rad,
...
def _sim_speckle_residual_render(cfg, sim_params, materials, n_frames: int,
                                 r_lo_mm: float, r_hi_mm: float,) -> dict:
    ...
    uses coherent_scatter=True so the sim's per-frame
    ...
    dtheta_rad = 2.0 * math.pi / n_theta
    return _speckle_residual_metrics(bg_frames, r_mm_sim, dtheta_rad=dtheta_rad,
        r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm)
...
def _test_speckle_residual_anchors(cfg, sim_params, materials, n_frames: int,
                                   ...
                                   r_lo_mm: float = 3.0,
                                   r_hi_mm: float = 6.5,) -> tuple[dict, dict, dict]:
    """Phase 3 primary speckle anchor: compare sim vs bench
    *residual-domain* radial / lateral correlation lengths.
    ...
    the same operating point the noise anchor uses, where the
    ...
    sim_params.gain_db = saved_gain_db + gain_bump_db
    try:
        sim_metrics = _sim_speckle_residual_render(cfg, sim_params, materials, n_frames,
            r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm)
    finally:
        sim_params.gain_db = saved_gain_db

    bench_primary = _bench_speckle_residual_e4(bench_slider, diameter_mm,
        r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm, phantom="e4c", take="auto")
    bench_cross = _bench_speckle_residual_e4(bench_slider, diameter_mm,
        r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm, phantom="e4a", take="auto")

    rel_err_radial = (abs(sim_metrics["radial_corr_mm"] - bench_primary["radial_corr_mm"])
        / max(bench_primary["radial_corr_mm"], 1e-9))
    rel_err_lateral = (abs(sim_metrics["lateral_corr_arc_mm"] -
            bench_primary["lateral_corr_arc_mm"])
        / max(bench_primary["lateral_corr_arc_mm"], 1e-9))
    rel_err_residual_std = (abs(sim_metrics["residual_std_palette"]
            - bench_primary["residual_std_palette"])
        / max(bench_primary["residual_std_palette"], 1e-9))

    cross_consistency_rel = (abs(bench_primary["radial_corr_mm"] - bench_cross["radial_corr_mm"])
        / max(bench_primary["radial_corr_mm"], 1e-9))
...
def _test_speckle_mu0_invariance(cfg, sim_params, materials, n_frames: int,
    ...
    try:
        for scale in mu0_scales:
            ...
            metrics = _sim_speckle_residual_render(cfg, sim_params, materials, n_frames,
                r_lo_mm=r_lo_mm, r_hi_mm=r_hi_mm)
            per_scale.append({
    ...
    if nom_radial > 0.0 and all(np.isfinite(radial_corrs)):
        max_rel_shift = float(max(abs(rc - nom_radial) / nom_radial for rc in radial_corrs))
    else:
...
def _render_speckle_residual_autocorr_paired_figure(*, bench_e4c: dict, bench_e4a: dict, sim: dict,
    comparison: dict, mu0_result: dict, out_path: Path,) -> None:
    """Paired figure for Test M's residual-domain spatial autocorrelation
    primary metric.
    ...
    # --- Radial autocorr ---
    for label, color, m in (("bench E4c (primary)", "C0", bench_e4c),
    ...
    fig.suptitle("Test M -- residual-domain spatial autocorrelation paired figure "
...
def test_speckle_wave0(cfg, sim_params, materials, n_frames: int,
                       ...
                       bench_slider: float = 68.0,
                       ref_slider: float = 54.0) -> TestResult:
    """M — uniform-milk speckle: reformulation (residual-domain
    ...
    backwards compatibility with the anchor and to surface any
    ...
    primary_detail: dict
    try:
        bench_p, bench_x, sim_resid, comparison = _test_speckle_residual_anchors(cfg, sim_params, materials, n_frames,
            bench_slider=bench_slider, ref_slider=ref_slider,
            diameter_mm=15.0, r_lo_mm=3.0, r_hi_mm=6.5)
    except (FileNotFoundError, RuntimeError, ValueError) as exc:
        return TestResult("M. Speckle (residual-domain milk anchor)",
            "n/a",
    ...
    primary_summary = (f"Residual radial_corr: sim {sim_resid['radial_corr_mm']:.3f} mm "
    ...
    try:
        mu0_result = _test_speckle_mu0_invariance(cfg, sim_params, materials, n_frames,
            bench_slider=bench_slider, ref_slider=ref_slider,
            r_lo_mm=3.0, r_hi_mm=6.5,
            mu0_scales=(1.0, 2.0))
        mu0_ok = bool(mu0_result["invariance_ok"])
        mu0_summary = (f"mu0-invariance: max_rel_shift "
            f"{mu0_result['max_rel_shift_radial_corr']*100:.1f}% "
            f"(≤20% tol; {'✓' if mu0_ok else '✗'}). "
            f"per_scale: " + ", ".join(f"x{r['mu0_scale']:.1f}->{r['radial_corr_mm']:.3f}mm"
    ...
    # follow-up: paired residual autocorr figure.
    speckle_fig_path: str | None = None
    if plt is not None:
        try:
            fig_path = out_dir / "figures" / "speckle_residual_autocorr_paired.png"
            fig_path.parent.mkdir(parents=True, exist_ok=True)
            _render_speckle_residual_autocorr_paired_figure(bench_e4c=bench_p, bench_e4a=bench_x, sim=sim_resid,
    ...
    return TestResult("M. Speckle (residual-domain milk anchor + mu0-invariance)",
        status, summary, detail)


def test_tgc(cfg, sim_params, materials, n_frames: int, out_dir: Path) -> TestResult:
    """H -- true simulator round-trip of the TGC schedule.
    ...
    Why this catches the C++ bug. Before the fix the kernel's
    ...
    # the kernel bug, so this test specifically guards against its
    ...
    def _render_with_tgc(tgc_cps):
        # note: `sim_params.tgc_control_points = [...]` is a Python-side
    ...
    if plt is not None:
        try:
            ...
            ax.set_title(f"Test H -- recovered vs expected Delta_TGC ({badge})\n"
    ...
    if pass_ok:
        return TestResult("H. TGC schedule",
            "pass",
...
# Writeup
# ----------------------------------------------------------------------------
# Phase 4: "diagnostic" is the status emitted by phenomenology-bank
...
def _bench_evidence_fig_link(key: str, caption: str,
    ...
    if explicit_path is not None:
        ...
        try:
            rel_to_report = full.resolve().relative_to((HERE / "tier1_results").resolve())
...
def _append_log_multiplier_section(lines: list[str]) -> None:
    ...
    lines.append("\n**Evidence figure.**\n\n")
    lines.append(_bench_evidence_fig_link("log_multiplier",
...
def _append_bench_evidence_section(lines: list[str], psf_anchor: dict | None) -> None:
    ...
    if psf_anchor is not None:
        ...
        lines.append(_bench_evidence_fig_link("psf_per_wire_patches",
        ...
        lines.append(_bench_evidence_fig_link("psf_per_wire_axial_profiles",
        ...
        lines.append(_bench_evidence_fig_link("psf_per_wire_lateral_profiles",
        ...
        lines.append(_bench_evidence_fig_link("psf_gaussian_beam_fit",
        ...
        lines.append(_bench_evidence_fig_link("psf_axial_fwhm_histogram",
    ...
    lines.append("\n**Extraction protocol.** `derive_ringdown_v2.py` polar-unwraps "
        ...
        "slope.\n")
    lines.append("\n**Evidence figure.**\n\n")
    lines.append(_bench_evidence_fig_link("ringdown",
    ...
    lines.append("\n**Evidence figure.**\n\n")
    lines.append(_bench_evidence_fig_link("noise",
    ...
    lines.append("\n**Evidence figure.**\n\n")
    lines.append(_bench_evidence_fig_link("depth_uniformity",
    ...
    lines.append("\n**Evidence figure.**\n\n")
    lines.append(_bench_evidence_fig_link("speckle",
    ...
    lines.append("\n**Evidence figure.**\n\n")
    lines.append(_bench_evidence_fig_link("tgc",
...
def render_markdown(results: list[TestResult], cfg, n_frames_wire: int,
    ...
    if psf_anchor is not None:
        lines.append(f"**Bench PSF anchor (tests C / D):** `{psf_anchor['label']}`. "
        ...
        if psf_anchor.get("patches_figure") is not None and psf_anchor["patches_figure"].is_file():
            ...
            lines.append(f"**Bench-data visual sanity checks:** per-wire 2D patches "
    ...
    for r in results:
        ...
        if r.name.startswith("C.") or r.name.startswith("D."):
            ...
            lines.append("*Measurement protocol.* We walk the −6 palette FWHM through "
            ...
            if tbl:
                ...
                if gd.get("available"):
                    ...
                    for w in gd.get("per_wire_sim", []) or []:
                        lines.append(f"| {w.get('r_mm', float('nan')):.0f} | "
                ...
                if r.name.startswith("D."):
                    ...
                    bench_ang_target_deg = math.degrees(float(cfg.processing.lateral_psf_kernel.sigma_theta_rad)
                        * 2.0 * math.sqrt(2.0 * math.log(2.0))) if cfg is not None and getattr(cfg.processing, "lateral_psf_kernel", None) else None
        ...
        if r.name.startswith("E"):
            ...
            if per_pair:
                lines.append("| (gain, D) | sim gain_db | sim peak | bench peak | rel.err | sim extent | bench extent | extent err | RMS inner 3 mm |\n"
                    "|---|---:|---:|---:|---:|---:|---:|---:|---:|\n")
                for pr in per_pair:
                    lines.append(f"| ({pr['gain']:.0f}, {pr['diameter_mm']:.0f}) "
        ...
        if r.name.startswith("F."):
            ...
            if per_anchor:
                ...
                for a in per_anchor:
                    ...
                    vf_e4c = float(bench_e4c.get("envelope_valid_fraction", 0.0) or 0.0)
                    lines.append(f"| {a.get('slider', '?')} | "
            ...
            if per_pair:
                ...
                for pr in per_pair:
                    ...
                    lines.append(f"| g{pr['gain']:.0f} D{pr['diameter_mm']:.0f} | "
                        f"{pr['sim_gain_db']:+.1f} | "
                        f"{sim_lc:.2f} mm | "
                        f"{bench_lc:.2f} mm | "
                        f"{corr_rel_str} | "
                ...
                if skipped:
                    skipped_strs = ", ".join(f"g{s['gain']:.0f}D{s['diameter_mm']:.0f} "
        ...
        if r.name.startswith("B2."):
            ...
            lines.append(f"| B2a (function check) | "
            ...
            lines.append(f"| B2b (sim-internal 6 dB Δ) | "
            ...
            lines.append(f"| B2c (E2E milk slider 68) | "
        ...
        if r.name.startswith("I."):
            ...
            lines.append("| Quantity | Value | Tolerance | Tier |\n|---|---:|---:|---|\n"
        ...
        if r.name.startswith("M.") and not r.name.startswith("M-"):
            ...
            radial_rel = float(comparison.get("rel_err_radial_corr_mm",
                                              float("nan")))
            lines.append(f"| radial_corr_mm | {radial_sim:.3f} | {radial_e4c:.3f} | "
            ...
            lat_rel = float(comparison.get("rel_err_lateral_corr_arc_mm",
                                            float("nan")))
            lines.append(f"| lateral_corr_arc_mm | {lat_sim:.3f} | {lat_e4c:.3f} | "
            ...
            resid_rel = float(comparison.get("rel_err_residual_std_palette",
                                             float("nan")))
            lines.append(f"| residual_std_palette (diag) | {resid_sim:.3f} | "
            ...
            if per_scale:
                lines.append("\n**mu0-invariance cross-check (gates Test M):**\n\n"
                    ...
                    "|---:|---:|---:|---:|---:|\n")
                for r_ in per_scale:
                    lines.append(f"| x{r_.get('mu0_scale', 0):.1f} | "
                        f"{r_.get('mu0', 0):.3f} | "
    ...
    lines.append("\n## Bench data requests\n")
    lines.append("All eleven Tier 1 parameter-bank tests currently pass against the "
...
def main() -> int:
    ...
    ap.add_argument("--psf-anchor", choices=("p035", "wave0"), default=PSF_ANCHOR_DEFAULT,
    ...
    ap.add_argument("--skip-speckle", action="store_true",
    ...
    def _append_with_companions(parent: TestResult) -> None:
        """Append the parent (bank=parameter by default) and any derived
        phenomenology-bank companions extracted
    ...
    _append_with_companions(test_ringdown(cfg, sim_params, materials, args.n_frames_anechoic, out_dir))
    _append_with_companions(test_noise(cfg, sim_params, materials, args.n_frames_anechoic, out_dir=out_dir))
    ...
    _append_with_companions(test_depth_uniformity(cfg, sim_params, materials, args.n_frames_anechoic, out_dir))
    if not args.skip_speckle:
        _append_with_companions(test_speckle_wave0(cfg, sim_params, materials, args.n_frames_anechoic, out_dir))
